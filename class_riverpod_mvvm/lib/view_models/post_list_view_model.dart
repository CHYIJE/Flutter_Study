...
class PostListViewModel extends StateNotifier<List<Post>> {
  ...
  // 통신 요청을 통해서 데이터를 가져오는 비즈니스 로직을 담당 시킬것이다.
  final PostRepository _postRepository;

  // T state --> List<Post> <---
  // 맨 처음 부모 클래스 StateNotifier를 가지고 있는 PostListViewModel의 상태는
  // 당연히 빈 값을 들고있다.
  ...
  Future<void> deletePost(int id) async {
    try {
      ...
      // 리스트 10개 중에 1개를 삭제햇음 --> List<Post> 값 상태가 변경이 되었다.
      // state --> [Post(), Post(), Post()];
      // 상태가 변경이 되면 새로운 리스트 객체를 state 변수에 넣어주어야
